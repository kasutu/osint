...
try:
    ...
    # Function to handle StaleElementReferenceException and print outerHTML
    def print_outer_html(element):
      attempt = 0
      while attempt < 3:  # Maximum of 3 attempts
          try:
              # Get the outerHTML attribute of the element
              outer_html = element.get_attribute('outerHTML')
              break
          except StaleElementReferenceException:
              print(f"Element is stale. Re-finding and retrying. Attempts: {attempt}")
              attempt += 1
